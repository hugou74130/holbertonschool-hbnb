"""
populate.py — Seed the HBnB in-memory database with test data.
...
The script:
  1. Creates the first user (auto-promoted to admin).
...
def main():
    ...
    # 1. Admin user (first user auto-admin, no token needed)
    ...
    wifi_id    = amenities.get("WiFi",             {}).get("id")
    pool_id    = amenities.get("Pool",             {}).get("id")
    parking_id = amenities.get("Parking",          {}).get("id")
    ac_id      = amenities.get("Air conditioning", {}).get("id")
    kitchen_id = amenities.get("Kitchen",          {}).get("id")
    pet_id     = amenities.get("Pet-friendly",     {}).get("id")
    ...
    # Login helper to get tokens for regular users (needed to post reviews)
    ...
    # Reviews: reviewer must not be the owner of the place
    reviews_data = [
        # place 0 (owner=alice): bob and charlie review
        ...
        # place 1 (owner=bob): alice and charlie review
        ...
        # place 2 (owner=charlie): alice and bob review
        ...
        # place 3 (owner=alice): bob reviews
